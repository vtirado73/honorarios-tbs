...
import { parseExcel, importAsignaturas } from '../../modules/asignaturas/services/asignaturaImportService'
import { carreraService } from '../../modules/carreras/services/carreraService'

export default function AsignaturaImportar() {
  ...
  useEffect(() => {
    carreraService.getAll()
      .then(data => setCarreras(data.filter(c => c.active)))
      .catch(() => {})
  }, [])

  const careerMap = Object.fromEntries(carreras.map(c => [c.acronym, c.name]))
  ...
  async function handleUpload() {
    ...
    try {
      const res = await importAsignaturas(rows)
      setResult(res)
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Suba un archivo Excel con los datos de las asignaturas
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        {result && (
          <div className={`mb-4 px-4 py-3 rounded-lg text-sm ${
            ...
            <p>{result.success} asignatura{result.success !== 1 ? 's' : ''} importada{result.success !== 1 ? 's' : ''} correctamente.</p>
            {result.errors.length > 0 && (
