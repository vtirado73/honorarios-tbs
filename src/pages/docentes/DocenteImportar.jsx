...
import { parseExcel, importDocentes } from '../../modules/docentes/services/docenteImportService'

export default function DocenteImportar() {
  ...
  async function handleUpload() {
    ...
    try {
      const res = await importDocentes(rows)
      setResult(res)
      if (res.errors.length === 0) {
        setTimeout(() => navigate('/docentes'), 1500)
      }
    } catch {
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Suba un archivo Excel con los datos de los docentes
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        {result && (
          <div className={`mb-4 px-4 py-3 rounded-lg text-sm ${
            ...
            <p>{result.success} docente{result.success !== 1 ? 's' : ''} importado{result.success !== 1 ? 's' : ''} correctamente.</p>
            {result.errors.length > 0 && (
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {rows.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-3 py-2 text-gray-900 dark:text-white">{row.lastname}</td>
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{row.surname || '—'}</td>
                      <td className="px-3 py-2 text-gray-900 dark:text-white font-medium">{row.ci}</td>
                      ...
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{row.bank_name || '—'}</td>
                      <td className="px-3 py-2 text-gray-600 dark:text-gray-300">{row.bank_account || '—'}</td>
