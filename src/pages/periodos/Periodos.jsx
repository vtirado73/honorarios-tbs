...
import { usePeriodos } from '../../modules/periodos/hooks/usePeriodos'
...
export default function Periodos() {
  ...
  const { periodos, loading, error, toggleActive } = usePeriodos()

  async function handleToggleActive(periodo) {
    const label = periodo.active ? 'desactivar' : 'activar'
    if (!window.confirm(`¿Estás seguro de ${label} el periodo "${periodo.name}"?`)) return
    try {
      await toggleActive(periodo.id)
    } catch {
  ...
  if (error) {
    return (
      <div className="p-4 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg">
        Error al cargar periodos: {error}
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Periodos Académicos</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Administración de periodos académicos
          </p>
        </div>
        <button
          onClick={() => navigate('/periodos/registro')}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-gray-400">No hay periodos registrados</td>
                </tr>
              ) : (
                periodos.map((p, i) => (
                  <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{formatDate(p.start_at)}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-300">{formatDate(p.end_at)}</td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => navigate(`/periodos/editar/${p.id}`)}
